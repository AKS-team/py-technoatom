...
sys.path.append(os.path.join(BASE_DIR, 'task_tracker'))

print(sys.path)

def pytest_configure(config):
    if not settings.configured:
        os.environ['DJANGO_SETTINGS_MODULE'] = 'task_tracker.task_tracker.settings'

    settings.DATABASES['default'] = settings.TEST_DATABASES[os.environ.get('DJANGO_TEST_DATABASE', 'default')]
    settings.ROOT_URLCONF = 'task_tracker.task_tracker.urls'
